...
const AddReadingSessionModal = ({ isModalOpen, setIsModalOpen, book, setBook, readingSessionList, setReadingSessionList }) => {
	...
	const handleAddReadingSession = (e) => {
		...
		addReadingSession(book.id, readingSession)
			.then((success) => {
				if (success) {
	...
	const predictReadtime = () => {
		...
		const predictedReadTime = Math.round(averageReadTimePerPage * readPage)

		setReadTime(predictedReadTime)
		const readTimeElement = document.getElementById('read-time-input') as HTMLInputElement
		if (readTimeElement !== null) {
			readTimeElement.blur()
			readTimeElement.value = String(predictedReadTime)
		}
